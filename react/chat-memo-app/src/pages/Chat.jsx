...
export default function Chat() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const aiText =
        response.text || response?.text() || "AI 응답을 불러올 수 없습니다.";

      let parsed;
      try {
        parsed = JSON.parse(aiText);
        if (typeof parsed.isMemo !== "boolean") parsed.isMemo = false;
        if (!parsed.content) parsed.content = aiText;
        if (!parsed.dueDate) parsed.dueDate = null;
      } catch {
        parsed = { isMemo: false, content: aiText, dueDate: "날짜 정보 없음" };
      }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f9f5e7] font-serif">
      {/* 상단 제목 */}
      <div className="bg-[#e8dcc0] border-b border-[#d4c8aa] text-center py-4 shadow-sm">
        <h1 className="text-3xl font-bold text-[#5c4b3b]">Chat Memo</h1>
        <p className="text-sm text-[#8a7c67]">AI와 함께 메모를 작성해보세요</p>
      </div>

      {/* 채팅 내역 */}
      <div className="flex-1 overflow-y-auto p-4 bg-[#fbf8ef]">
      ...
      {/* 입력창 */}
      <div className="border-t border-[#d8ccb0] bg-[#f5eedc] p-4 shadow-inner">
