...
// Boolean() 함수: 불리언으로 형변환 하는 도구
console.log(Boolean("김새하"));
...
console.log(Boolean(-0)); // false // js에는 -0있다.
console.log(Boolean(99999999));

// 문자열 형변환
console.log(Boolean("")); // false (비어있는 것)
console.log(Boolean("     ")); // true (공백은 가능)

// 암시적 불리언 형변환
// 불리언 데이터가 아닌 데이터에 대한 논리 연산 (&&. ||, !)

// 0 --암시적 불리언 형변환--> false --not연산--> true
console.log(!0); // true
console.log(!""); // true
console.log(!1); // false

// 정확한 논리연산자 작동 방식
// AND(&&): 2개의 데이터가 모두 true라면 오른쪽 데이터 생성
// true && true
console.log(1 && 2); // 2

// OR(||): 왼쪽 데이터가 true면 왼쪽 데이터 생성,
// 왼쪽 데이터가 false면 오른쪽 데이터 생성
// true || false
console.log(1 || 0); // 1
// false || true
console.log(0 || 10); // 10
